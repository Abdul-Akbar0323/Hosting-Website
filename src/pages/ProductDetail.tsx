...
type SectionId = 'details' | 'shipping' | 'specs';
...
export default function ProductDetail() {
          ...
          <div className="rounded-3xl border border-nova-200 bg-white p-6 shadow-sm">
            <div className="space-y-6">
              <div>
                ...
                <h1 className="mt-2 text-3xl font-bold text-nova-900">
                  {product.name}
                </h1>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <span className="text-sm text-nova-500">
                  {reviews.length} reviews
                </span>
              </div>

              <div className="flex flex-wrap items-center gap-3">
                <span className="text-3xl font-semibold text-nova-900">
                  {formatPrice(product.price)}
                </span>

                {product.originalPrice && (
                  <span className="text-sm text-nova-500 line-through">
                    {formatPrice(product.originalPrice)}
                  </span>
                )}
              </div>

              <p className="text-sm leading-relaxed text-nova-600">
                      ...
                      {product.sizes.map((size) => (
                        <button
                          ...
                          className={`rounded-2xl px-4 py-2 text-sm border transition ${
                            selectedSize === size
                              ? 'border-nova-900 bg-nova-900 text-white'
                              : 'border-nova-300 text-nova-700 hover:border-nova-500'
                          }`}
                        >
                          {size}
                        </button>
                ...
                <div>
                  ...
                  <div className="inline-flex items-center rounded-full border border-nova-300 overflow-hidden">
                    <button
                      ...
                      className="px-4 py-3 text-nova-700 hover:bg-nova-50"
                    >
              ...
              <div className="flex flex-wrap items-center gap-4 pt-2 text-sm">
                <button
                  ...
                  className="flex items-center gap-2 text-nova-600 hover:text-nova-900"
                >
                  <Heart
                    className={`w-4 h-4 ${
                      inWishlist
                ...
                <button
                  type="button"
                  className="flex items-center gap-2 text-sm text-nova-600 hover:text-nova-900"
                >
              ...
              <div className="grid grid-cols-3 gap-4 pt-6 border-t border-nova-100">
                <div className="text-center">
                  ...
                  <p className="text-xs text-nova-600">
                    Free Shipping
                    <br />
                    over $100
                  </p>
      ...
      <div className="mt-12 lg:mt-20 max-w-7xl mx-auto">
        <div className="border-t border-nova-200 pt-10">
          {sections.map((section) => (
            <div
              key={section.id}
              className="border-b border-nova-200 py-5"
            >
              <button
                type="button"
                onClick={() =>
                ...
                className="w-full flex items-center justify-between text-left text-nova-900"
              >
                ...
                {expandedSection === section.id ? (
                  <ChevronUp className="w-5 h-5 text-nova-500" />
                ) : (
                  <ChevronDown className="w-5 h-5 text-nova-500" />
                )}
              </button>

              {expandedSection === section.id && (
                <div className="pt-4 text-nova-600">
                  {section.content}
                </div>
              )}
            </div>
        ...
        <div className="mt-12">
          <h2 className="text-2xl font-bold text-nova-900 mb-8">
            Customer Reviews
          </h2>
          ...
          {reviews.length > 0 ? (
            <div className="grid gap-6 md:grid-cols-2">
              {reviews.map((review) => (
                <div
                  key={review.id}
                  className="bg-nova-50 p-6 rounded-3xl"
                >
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      <p className="font-medium text-nova-900">
                        {review.userName}
                      </p>
  ...
  size="sm"
/>
                  <h4 className="font-semibold text-nova-900 mt-3">
                    {review.title}
                  </h4>

                  <p className="text-sm leading-relaxed text-nova-600 mt-2">
                    {review.content}
                  </p>
                </div>
              ))}
